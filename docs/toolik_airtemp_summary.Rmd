...
```{r}
toolik_mean_temp_jitter <- ggplot(data = toolik_weather, aes(x = month, 
                                                             y = daily_air_temp_mean_c)) + 
  geom_jitter(aes(color = daily_air_temp_mean_c), 
              show.legend = FALSE) + 
  labs(x = "Month", 
       y = "Daily Mean Air Temperature (Celcius)", 
       title = "Toolik Station Daily Air Temperature") + 
  scale_color_gradient(low = "blue", high = "orange")

ggsave(here("figs", "toolik_mean_temp_jitter.png"), height = 5, width = 6)
```
